...
import PlaylistReviewModal from './playlistReviewModal';

const PlayListPage = () => {
    const [playlists, setPlaylists] = useState([]);
    const [selectedPlaylist, setSelectedPlaylist] = useState(null);
    const [showReviewModal, setShowReviewModal] = useState(false);
    const [reviewTargetTrack, setReviewTargetTrack] = useState(null);
    const navigate = useNavigate();

    // 리뷰 모달이 열리면 디테일 모달 닫기
    useEffect(() => {
        if (showReviewModal) {
        ...
        <div>
            ...
            {showReviewModal && reviewTargetTrack && (
                <PlaylistReviewModal
                    ...
                    playlist_music_name={reviewTargetTrack.trackName}
